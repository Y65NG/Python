...
def extendEuclidean(e: int, phi: int):
    ...
    >>> assert (d * 14528as/ad + k * 360931 == 1), "extendEuclidean() Fail to past Test case #2"
    """
...
def generateMyKeys():
	...
	privateKey = extendEuclidean(publicKey, calcPhi(p, q))[0]
	n = p * q
	""" WRITE YOUR CODE ABOVE """
...
def encryptObject(msg_object: object, publicKey: int, n: int):
	return [
		encryptInt(token, publicKey, n) for token in longMsgIterator(msg_object)
	]
...
def decryptObject(msg_list: list, privateKey: int, n: int):
	...
	for token in msg_list:
		token_combiner.addToken(token, privateKey, n)
