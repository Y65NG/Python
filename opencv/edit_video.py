...
def generatePicture(path,maxNum):  # path是保存图片的文件夹的名字
    ...
    # cap.release()
    face_cc = cv2.CascadeClassifier("haarcascade_frontalface_default.xml") # 分类器,从图片中按照特征值进行分类.""里面的就是人脸的特征值
    num=0
    ...
    while cap.isOpened():
        ...
        if len(ifFaces) >0:
            for (x,y,w,h) in ifFaces:   # x,y表示的是所选区域的左下角,x为向右的宽,h为向上的高
                cv2.rectangle(img,(x,y),(x+w,y+h),(255,0,0),2)    # 在找到的人脸上画一个矩形框出来
                num=num+1
                if num>maxNum:
                    break
                else:
